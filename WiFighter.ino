/*
 * WiFighter v0.6 - Portable WiFi/BLE Device Data Recon Tool
...
void setup() {
  ...
  updateBattery();
  drawHome();
  Serial.println("[WiFighter v0.6] Boot complete - Authorized use only");
}
...
void loop() {
  ...
  if (probeSniffing) {
    ...
    if (now - lastProbeUi > 1100) {
      drawProbeSniff();
      lastProbeUi = now;
      lastActivity = now;   // keep alive while sniffing
    }
  }
    ...
    switch (currentState) {
      ...
      case STATE_MENU:
        switch (menuIndex) {
          ...
          case 4: // View Results – prefer hybrid view if both, else single
...
void drawHome() {
  ...
  M5.Display.print("BLE + WiFi Recon  v0.6");
...
void drawProbeSniff() {
  ...
  if (probeResults.empty()) {
    ...
    for (int i = (int)probeResults.size() - 1; i >= 0 && shown < 5; i--) {
      ...
      M5.Display.setTextColor(COL_TEXT);
      M5.Display.setCursor(4, y);
      String macShort = e.mac.substring(9); // last 8 chars
...
void drawAbout() {
  ...
  M5.Display.print("WiFighter v0.6");
...
void IRAM_ATTR wifi_sniffer_cb(void* buf, wifi_pkt_rx_ctrl_t* ctrl) {
  ...
  while (pos + 2 < len) {
    ...
    if (tag == 0x00 && tagLen > 0 && tagLen < 33) {
      char ssidBuf[33] = {0};
      memcpy(ssidBuf, &payload[pos + 2], tagLen);
      ssid = String(ssidBuf);
      break;
    }
    pos += 2 + tagLen;
  ...
  if (seenProbeMACs.find(mac) == seenProbeMACs.end()) {
    seenProbeMACs.insert(mac);
    ProbeEntry e;
    e.mac = mac;
    e.ssid = ssid;
    e.rssi = rssi;
    e.lastSeen = millis();
    if (probeResults.size() >= MAX_PROBES) probeResults.erase(probeResults.begin());
...
void startProbeSniff() {
  ...
  scanning = false;
  drawProbeSniff();
  Serial.println("[Probe] Continuous probe-request sniff started");
}
