/*
 * WiFighter v1.3 - Portable WiFi/BLE Device Data Recon Tool
...
void drawHome() {
  ...
  // Title row
  M5.Display.setTextDatum(TL_DATUM);
  M5.Display.setCursor(6, 17);
  M5.Display.setTextSize(2);
  M5.Display.setTextColor(TFT_CYAN);
  M5.Display.print("HOME");
  M5.Display.setTextSize(1);
  M5.Display.setCursor(100, 22);
  ...
  String strongestMac = "";
  for (auto& d : devices) {
    if (d.left) ghosts++; else {
      active++;
      if (d.rssi > strongest) {
        strongest = d.rssi;
        strongestMac = d.mac.length() > 8 ? d.mac.substring(d.mac.length() - 8) : d.mac;
      }
    }
  ...
  // Visual bar
  int barMaxW = 100, barY = 78;
  M5.Display.drawRect(6, barY, barMaxW, 8, TFT_DARKGREY);
  if (devices.size() > 0) {
    int actW = (active * barMaxW) / (int)devices.size();
    if (actW > 0) M5.Display.fillRect(7, barY + 1, actW - 1, 6, TFT_GREEN);
    if (actW < barMaxW) M5.Display.fillRect(6 + actW, barY + 1, barMaxW - actW - 1, 6, TFT_ORANGE);
  }

  // Last scan + radio mix
  M5.Display.setTextColor(TFT_LIGHTGREY);
  M5.Display.setCursor(6, 92);
  if (lastScanTime == 0) M5.Display.print("Scan: never");
  else M5.Display.printf("Scan: %lus ago", (millis() - lastScanTime) / 1000);

  M5.Display.setCursor(6, 105);
  M5.Display.printf("BLE:%d  WiFi:%d", bleCount, wifiCount);

  // Strongest device hint
  if (active > 0 && strongestMac.length() > 0) {
    M5.Display.setCursor(6, 118);
    M5.Display.setTextColor(TFT_YELLOW);
    M5.Display.printf("Near %s %ddBm", strongestMac.c_str(), strongest);
  }
...
void drawMenu() {
  ...
  // Quick status line under menu
  int active = 0, ghosts = 0;
  for (auto& d : devices) { if (d.left) ghosts++; else active++; }
  M5.Display.setTextColor(TFT_DARKGREY);
  M5.Display.setCursor(6, M5.Display.height() - 26);
  M5.Display.printf("P:%d  G:%d", active, ghosts);
...
void clearGhosts() {
  devices.erase(std::remove_if(devices.begin(), devices.end(), [](const TrackedDevice& d){ return d.left; }), devices.end());
}
...
void playTone(int freq, int duration) {
  // M5StickC Plus2 has speaker on some variants; soft tone via beep if available
  // Fallback silent for compatibility
}
