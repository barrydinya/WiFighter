...
- **Home Screen** – battery, tracked / present / departed counts with visual bar, BLE vs WiFi split, last-scan age, ready status
- **Main Menu** – button-driven with highlight selection (8 entries)
...
**v1.2** – Polished Home Screen + full Menu system, WiFi/BLE scanners, Track Mode with departed-device (ghost) detection + audio alert, dedicated Ghosts list, Settings (clear / export / sleep / brightness), NVS persistence, serial CSV export, proximity highlighting, OUI lookup. Ready for authorized field testing.
